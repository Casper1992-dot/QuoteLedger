...
├── server.js           ← serves the page + forwards AI calls (Claude or Hugging Face) with your key
...
|---|---|---|
| `ANTHROPIC_API_KEY` | Yes, for the AI features (or use `HF_TOKEN`, see below) | Your API key. Without it the page still works on the reference data, but Claude features are greyed out. |
...
## Free alternative: Hugging Face instead of Claude

The server can use Hugging Face's free monthly inference credits instead of an Anthropic key.

1. Create a free account at https://huggingface.co/join and verify your email.
2. Go to **Settings → Access Tokens → Create new token**. Choose **Fine-grained**, tick **“Make calls to Inference Providers”**, give it a name, and click **Create**. Copy the token; it starts with `hf_`.
3. On your host (Render/Replit), **remove** `ANTHROPIC_API_KEY` if it is set, and add:
   - `HF_TOKEN` = your `hf_…` token
   - `HF_MODEL` = `meta-llama/Llama-3.3-70B-Instruct` (the default), or another chat model that shows **Inference Providers** on its Hugging Face page
   - Optional: `HF_VISION=1` together with a vision model (for example a Qwen “-VL” instruct model) to send photos as images. Without it, photos still work through the in-browser text reader.
4. Redeploy and open `/api/health`. It should show `"provider":"huggingface"`.

**Limits to know:**
- **Small free allowance.** Free accounts get a small monthly credit (US$0.10 at the time of writing), enough for a short demo but not for heavy use. When it runs out, the app shows a “credits used up” message until next month, unless you add credit or upgrade to PRO.
- **Lower accuracy.** Open models are less reliable than Claude at returning strict JSON and at reading messy quotes. Expect the occasional “did not return valid JSON — try again”, and more items sent to review.
- **Labels still say “Claude”.** The page text still says Claude even when Hugging Face is answering.
...
| Symptom | Fix |
|---|---|
| “Claude isn’t available in this view” | `/api/health` isn't OK. Set `ANTHROPIC_API_KEY` (or `HF_TOKEN`) and restart or redeploy. |
